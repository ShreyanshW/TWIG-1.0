...
const float servoConfig[SERVO_COUNT][5] = {
  ...
  {215, 400, 0, 180, 1.0},    // Wrist (higher speed for flourishes)
  {300, 550, 0, 90, 0.9}      // Gripper
};
...
const float homeAngles[SERVO_COUNT] = {90, 45, 60, 90, 45}; // Updated return position
...
const struct {
  ...
  {{  0, 70, 270, 90, 30}, 750, 0, "Approach object (gripper open)"},
  {{  0, 60, 250, 90, 30}, 400,  0, "Lower to object"},
  {{  0, 50, 240, 90, 80}, 300, 300, "Grab object (gripper closed)"},
  
  // Lift and Transport (Base 0° → 180°)
  {{ 30, 60, 60, 90,  80}, 800, 0, "Lift and start moving"},
  {{ 60, 60, 45, 135, 80}, 500, 0, "First wrist flourish"},
  {{ 90, 60, 60, 45,  80}, 800, 0, "Continue transport"},
  {{135,  0, 60, 90,  75}, 1000, 0, "Approach drop point"},
  
  // Place Sequence (Base 180°, Shoulder -60°, Elbow 50°)
  {{180, -30, 135, 90,  80}, 800, 0, "Position for drop"},
  {{180, -30, 135, 90,  0}, 500, 800, "Release object (gripper open)"},
  {{180, -30, 135, 90,  80}, 500, 0, "Release object (gripper close)"},
  {{180, 30, 135, 90,  80}, 300, 0, "Release object (gripper up)"},

  // Return to Pickup Position
  {{90, 45, 240, 90,  80}, 500, 0, "Retract arm to pickup position"},
  {{0, 45, 240, 90,  80}, 500, 0, "Retract arm to pickup position"},
  {{0, 45, 240, 90,  30}, 500, 0, "Place at pickup position"},
  
  // Return Home (Base 90°, Shoulder 30°, Elbow 30°)
  {{ 90, 45, 30, 90,  45}, 1500, 0, "Return home"}
};
