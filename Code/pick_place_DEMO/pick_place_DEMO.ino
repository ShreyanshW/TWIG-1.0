...
#define BASE_SPEED 50           // Reduced speed for smoother motion
...
const struct {
  ...
  {{  0, 70, 270, 90, 10}, 350, 0, "Approach object (gripper open)"},
  {{  0, 60, 250, 90, 0}, 200,  0, "Lower to object"},
  {{  0, 50, 245, 90, 80}, 150, 0, "Grab object (gripper closed)"},
    {{  0, 50, 90, 90, 80}, 150, 0, "Grab object (gripper closed)"},

  
  // Lift and Transport (Base 0° → 180°)
  {{70, 60, 90, 90,  80}, 400, 0, "Lift and start moving"},
  {{70, 60, 45, 135, 80}, 250, 0, "First wrist flourish"},
  {{ 90, 60, 60, 45,  80}, 400, 0, "Continue transport"},
  {{135,  0, 60, 90,  80}, 500, 0, "Approach drop point"},

  // Place Sequence (Base 180°, Shoulder -60°, Elbow 50°)
  {{180, -30, 135, 90,  80}, 400, 0, "Position for drop"},
  {{180, -30, 135, 90,  30}, 250, 0, "Release object (gripper open)"},
  
  // Return Home (Base 90°, Shoulder 30°, Elbow 30°)
  {{180,  -10, 60, 90,  30}, 500, 0, "Retract arm"},
    {{180,  10, 60, 90,  30}, 500, 0, "Retract arm"},
  {{ 90, 30, 30, 90,  45}, 750, 0, "Return home"}
};
